...
def parseSyntax(tokens, output_text):
    ...
    class program:
            ...
            def user_defined_function(tokens, start_idx):
                ...
                if is_token(tokens, start_idx, '}'):
                    ...
                    # Always check if statements were found in the function body
            ...
            # Class variable to track if main function has been seen
            main_function_seen = False
            # Class variable to track if all functions are valid
            ...
            def main_function(tokens, start_idx):
                ...
                if is_token(tokens, start_idx, 'gene'):
                  # Check if we've already seen the main function
                    ...
                    if is_token(tokens, start_idx, ';'):
                        output_text.insert(tk.END, f"Syntax Error at line {line_number}: Unexpected ;\n")
                        output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                        return False, None

                    if not is_token(tokens, start_idx, ')'):
                        output_text.insert(tk.END, f"Syntax Error at line {line_number}: Expected a close parenthesis\n")
                        output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                        return False, None
                    ...
                    start_idx = skip_spaces(tokens, start_idx)            
                    if not is_token(tokens, start_idx, '{'):
                        output_text.insert(tk.END, f"Syntax Error at line {line_number}: Missing open brace for gene function\n")
                        output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                        return False, None
                    ...
                    if is_token(tokens, start_idx, '}'):
                        ...
                        if not check_statements:
                            output_text.insert(tk.END, f"Syntax Error at line {line_number}: No statements found within the function\n")
                            output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                            return False, None
                        return True, start_idx + 1
                    else:
                ...
                # This is a user-defined function
                    ...
                    if program.main_function_seen:
                            output_text.insert(tk.END, f"Syntax Error at line {line_number}: User defined function before main function\n")
                            output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                            return False, None
    ...
    class statements:
        ...
        def inside_loop_statement(tokens, start_idx):
            """
            Parse inside_loop_statement according to the grammar:
            <inside_loop_statement>➜<contig_statement>
            <inside_loop_statement>➜<destroy_statement>
            <inside_loop_statement>➜λ (empty)
            
            Returns:
            - is_valid (bool): Whether the parsing was successful
            - next_idx (int): The index after parsing
            """
        ...
        def stimuli_statement(tokens, start_idx):
            ...
            # Check if this is an assignment-like statement
            if is_token(tokens, start_idx, '='):
                print("Detected stimuli statement")
                
                # DON'T move past '=' here, pass it as is to assignment_statement
                # Check if the token after equal sign is 'stimuli'
                next_idx = start_idx + 1
                next_idx = skip_spaces(tokens, next_idx)
                
                if not is_token(tokens, next_idx, 'stimuli'):
                    # Include the '=' token in the assignment processing
                    is_valid, new_idx = statements.assignment_statement(tokens, start_idx)
                    if not is_valid:
                        print(f"Error: Invalid assignment statement at index {start_idx}")
                        return False, None
                    print("tapos assignment")
                    return True, new_idx
                
                # If it's 'stimuli', continue with stimuli statement parsing
                start_idx += 1  # Now move past '='
                start_idx = skip_spaces(tokens, start_idx)
            ...
            # No '=' sign, check if it's 'stimuli'
            elif is_token(tokens, start_idx, 'stimuli'):
                # Process as a normal stimuli statement
                start_idx += 1  # Move past 'stimuli'
                start_idx = skip_spaces(tokens, start_idx)
            ...
            # Neither '=' nor 'stimuli', try to parse as an assignment statement
            else:
                is_valid, new_idx = statements.assignment_statement(tokens, start_idx)
                if not is_valid:
                    print(f"Error: Invalid assignment statement at index {start_idx}")
        ...
        def assignment_statement(tokens, start_idx):
            output_text.insert(tk.END, "<assignment_statement>\n")
            ...
            while start_idx < len(tokens) and is_token(tokens, start_idx, ','):
                output_text.insert(tk.END, "<assignment_statement_tail>\n")
        ...
        def assignment_value(tokens, start_idx):
            output_text.insert(tk.END, "<assignment_value>➜")
            start_idx = skip_spaces(tokens, start_idx)
    ...
    class variables:
        ...
        def validate_doseval(tokens, start_idx):
                    ...
                    while True:
                                ...
                                if next_idx < len(tokens) and any(is_token(tokens, next_idx, op) for op in math_operator):
                                    ...
                                    if is_valid:
                                        ...
                                        if number_sign in {"quantval", "nequantliteral"}:
                                            output_text.insert(tk.END, f"Datatype error at line {line_number}: Expected an dose\n")
                                            output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                                            return False, None
                                        start_idx += 1
                                else:
                        ...
                        if not is_token(tokens, start_idx, ','):
                            output_text.insert(tk.END, f"Datatype error at line {line_number}: Expected a comma\n")
                            output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                            return False, None
                        ...
                        # Next identifier
                        if not is_token(tokens, start_idx, 'Identifier'):
                            output_text.insert(tk.END, f"Datatype error at line {line_number}: Expected an Identifier\n")
                            output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                            return False, None
                        start_idx += 1
                                ...
                                if next_idx < len(tokens) and any(is_token(tokens, next_idx, op) for op in math_operator):
                                    ...
                                    if is_valid:
                                        ...
                                        if number_sign in {"doseliteral", "neliteral"}:
                                            output_text.insert(tk.END, f"Datatype error at line {line_number}: Expected a quant\n")
                                            output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                                            return False, None
                                        start_idx += 1
                                else:
                                    ...
                                    number_sign = check_number_sign(tokens[start_idx])
                                    if number_sign in {"doseliteral", "neliteral"}:
                                        output_text.insert(tk.END, f"Datatype error at line {line_number}: Expected an quant\n")
                                        output_text.insert(tk.END, f"Line {line_number}: {line_text}\n")
                                        return False, None
                                    start_idx += 1
                            elif is_token(tokens, start_idx, 'Identifier'):
    ...
    class express:
        ...
        def express_array_value(tokens, start_idx, identifier):
            ...
            default_value = None
            if start_idx < len(tokens) and tokens[start_idx][1] == 'doseliteral':
